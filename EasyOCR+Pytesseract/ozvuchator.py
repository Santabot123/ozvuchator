...
EASY_OCR=eval(config['settings']['easyOCR'])
PYTESSERACT = eval(config['settings']['pytesseract'])
...
def download_tessdata(pt2t_lang_code):
    ...
    current_directory=pathlib.Path().resolve()
    print(f"Завантаження .traindata файлу для {Lang(pt2t_lang_code).asdict()['name']}. Зачекайте...")
    ...
    print(f"{Lang(pt2t_lang_code).asdict()['name']} завантажено")
    print("Список завантажених мов:",pytesseract.get_languages(config=''))
...
def auto_mode(LEFT, TOP, RIGHT, BOTTOM):
    ...
    while True :
        if win32api.GetAsyncKeyState(PAUSE_KEY)<0:
            ...
            display('НАТИСНУТО КЛАВІШУ ПАУЗИ')
...
class mywidget(QWidget):
    def __init__(self):
        ...
        self.ui.EasyOCR_radioButton.setChecked(eval(config['settings']['easyOCR']))
        if self.ui.EasyOCR_radioButton.isChecked():
            self.ui.Pytesseract_settings_groupBox.hide()
        self.ui.EasyOCR_radioButton.clicked.connect(self.switch_to_easyOCR)
        self.ui.Pytesseract_radioButton.setChecked(eval(config['settings']['pytesseract']))
        self.ui.Pytesseract_radioButton.clicked.connect(self.switch_to_pytesseract)
    ...
    def switch_to_pytesseract(self):
        self.ui.Pytesseract_settings_groupBox.show()
        self.ui.SPEAK_LANGUAGE_comboBox.clear()
        self.ui.SPEAK_LANGUAGE_comboBox.addItems(sorted(pytesseract_set.intersection(translator_set,speaker_set)))
        self.ui.SPEAK_LANGUAGE_comboBox.setCurrentText('en')
        self.ui.TRANSLATE_FROM_comboBox.clear()
        self.ui.TRANSLATE_FROM_comboBox.addItems(sorted(pytesseract_set.intersection(translator_set,speaker_set)))
        self.ui.TRANSLATE_FROM_comboBox.setCurrentText('en')


    def switch_to_easyOCR(self):
        self.ui.Pytesseract_settings_groupBox.hide()
        self.ui.SPEAK_LANGUAGE_comboBox.clear()
        self.ui.SPEAK_LANGUAGE_comboBox.addItems(sorted(easyocr_set.intersection(translator_set,speaker_set)))
        self.ui.SPEAK_LANGUAGE_comboBox.setCurrentText('en')
        self.ui.TRANSLATE_FROM_comboBox.clear()
        self.ui.TRANSLATE_FROM_comboBox.addItems(sorted(easyocr_set.intersection(translator_set,speaker_set)))
        self.ui.TRANSLATE_FROM_comboBox.setCurrentText('en')
    ...
    def apply_settings(self):
        ...
        config.set('settings','EASYOCR',str(self.ui.EasyOCR_radioButton.isChecked() ) )
        config.set('settings','PYTESSERACT',str(self.ui.Pytesseract_radioButton.isChecked() ) )
    ...
    def refresh_settings(self):
        global SPEAK_LANGUAGE,DETECTION_LANGUAGES,TRANSLATE,EASY_OCR,PYTESSERACT,TRANSLATE_FROM,TRANSLATE_TO,SPEED_UP,REDUCE_NOISE,OUTLINED,RESIZE,RESIZE_SCALE,PAUSE_KEY,MANUAL_MODE_KEY,IGNORE_WORDS,THRESHOLD
